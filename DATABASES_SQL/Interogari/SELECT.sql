...
WITH Q AS
(SELECT MAX(P.PretUnitar) AS Pret_Maxim, CAT.NumeCategorie AS NUME_CATEGORIE, CAT.IdCategorie AS ID_CAT
FROM CategoriiProduse AS CAT
INNER JOIN Produse AS P
ON P.IdCategorie = CAT.IdCategorie
GROUP BY CAT.NumeCategorie, CAT.IdCategorie)
SELECT Q.NUME_CATEGORIE, P.DenumireProdus, P.PretUnitar
FROM Produse AS P INNER JOIN Q
ON Q.ID_CAT = P.IdCategorie
WHERE P.PretUnitar = Q.Pret_Maxim
